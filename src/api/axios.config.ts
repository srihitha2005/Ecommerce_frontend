...
const createAPI = (
  ...
  console.log(
    `📡 [AxiosConfig] Creating ${serviceName} instance with URL:`,
    url,
  );
  return axios.create({
    baseURL: url,
...
const requestInterceptor = (config: InternalAxiosRequestConfig) => {
  ...
  if (token && config.headers) {
    ...
    console.log(
      `🔑 [Axios Interceptor] ${service} -> Attaching Token to: ${config.url}`,
    );
  } else {
    console.warn(
      `⚠️ [Axios Interceptor] ${service} -> No token found for request: ${config.url}`,
    );
  }
  return config;
};
